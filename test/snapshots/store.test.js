...
test("content-addressed store reuses identical semantic objects", async () => {
  ...
  try {
    ...
    const ir = createAnalysisIR({ scanContext: {}, facts: [] });
    const a = await store.putObject(ir);
    const b = await store.putObject(ir);
    assert.equal(a, b);
    assert.deepEqual(await store.getObject(a), ir);
...
test("dirty snapshots do not overwrite clean commit refs", async () => {
  ...
  try {
    const store = createSnapshotStore(repo);
    const base = { formatVersion: 1, semanticObjectHash: "a", scannedTreeHash: "t", scanConfigHash: "c", intentArtifacts: [], createdAt: "2026-01-01T00:00:00Z" };
