export async function resolveSnapshotSelector(store, selector) {
  ...
  const snapshots = await store.listSnapshots();
  const matches = snapshots.filter((item) => item.id === selector || item.id.startsWith(selector) || item.semanticObjectHash === selector || item.semanticObjectHash.startsWith(selector));
